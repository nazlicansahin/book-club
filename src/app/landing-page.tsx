...
export default function LandingPage() {
  ...
  useEffect(() => {
    if (searchParams.get("start") === "1" || searchParams.get("error")) {
      setStarted(true);
    }
  }, [searchParams]);
          ...
          <div className="w-full max-w-sm">
            {!started ? (
              <button
                ...
                disabled={loading}
                className="w-full pixel-border bg-tertiary text-on-tertiary text-xl font-bold font-[family-name:var(--font-space-mono)] uppercase px-12 py-6 pixel-shadow-lg transition-all duration-75 hover:translate-x-0.5 hover:translate-y-0.5 active:translate-x-1 active:translate-y-1 active:shadow-none disabled:opacity-60"
              >
                PRESS START
              </button>
            ) : (
              <div className="pixel-border bg-surface-container p-6 pixel-shadow text-left">
                <h2 className="text-lg font-bold font-[family-name:var(--font-space-mono)] text-primary uppercase mb-2 text-center">
                  Join the Quest
                </h2>
                <p className="text-sm text-on-surface-variant font-[family-name:var(--font-courier-prime)] mb-6 text-center">
                  Sign in or create your account, then choose your reader.
                </p>

                {!configured ? (
                  <div className="pixel-border bg-error-container text-on-error-container p-3 text-xs font-[family-name:var(--font-courier-prime)]">
                    Firebase is not configured. Add your NEXT_PUBLIC_FIREBASE_* env vars.
                  </div>
                ) : (
                  <>
                    <GoogleSignInButton onClick={signInWithGoogle} label="Continue with Google" />
                    <p className="mt-4 text-center text-[10px] font-bold font-[family-name:var(--font-space-mono)] text-outline uppercase tracking-widest">
                      New players &amp; returning readers use Google
                    </p>
                  </>
                )}
                ...
                <button
                  ...
                  className="mt-6 w-full text-xs font-bold font-[family-name:var(--font-space-mono)] text-on-surface-variant uppercase hover:text-primary transition-colors"
                >
                  ← Back
                </button>
              </div>
